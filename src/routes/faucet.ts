...
router.post('/', async (c) => {
  const { unlinkMnemonic, evmPrivateKey, target } = await c.req.json()
  const unlink = createUnlinkClient(unlinkMnemonic, evmPrivateKey)

  if (target === 'eoa') {
    const result = await unlink.faucet.requestTestTokens({ token: TOKEN })
    const confirmed = await unlink.pollTransactionStatus(result.txId)
    return c.json(confirmed)
  }

  if (target === 'unlink') {
    const result = await unlink.faucet.requestPrivateTokens({ token: TOKEN })
    const confirmed = await unlink.pollTransactionStatus(result.txId)
    return c.json(confirmed)
  }
